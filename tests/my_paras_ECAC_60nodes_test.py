...
version = '3.1.0' # Based on version 2.9.5 of Model 2.
...
file_net = jn(result_dir, 'networks/ECAC_60nodes_avg/ECAC_60nodes_avg.pic')
...
tau = 15.#1.*unit							# Increment of time for shifting flight plans.
...
file_traffic = jn(result_dir, 'networks/ECAC_60nodes_avg/ECAC_60nodes_avg_flights_selected.pic')
if file_traffic==None:
	...
	departure_times = 'from_data' # don't modify
	format_flights = '(n, z, t)'
	flows_index_based = True
	min_dis = 2
	bootstrap_mode = True	# if False, times of departures and entry/exits are exactly matched 
							# the data. Otherwise, they are generated from the distribution extracted 
							# on the data.
	if bootstrap_mode:
		bootstrap_only_time = False 	# if True, keeps the same number of flights (scaled down or scaled down if ACtot is specified)
