...
def per_AC_met_vs_par(list_results_r, paras, met='satisfaction', name='', rep='.',AC=[],  labelx='', labely='', loc=3, figsize=(10, 7), xlim=None, ylim=None,\
...
def get_results(paras, vers='3.0', Gname=None):
	if Gname==None:
		Gname = paras['G'].name

	results, results_global = loop({p:paras[p + '_iter'] for p in paras['paras_to_loop']}, 
									paras['paras_to_loop'], 
									paras, 
									{}, 
									{}, 
...
def loop(a, level, parass, results, results_global, vers=version, Gname=None):
	"""
	...
	level : list
		of parameters on which to loop. The first one is the most outer loop, the last one is the most inner loop.
	...
	if level==[]:
		...
		name = build_path_average(parass, Gname=Gname, rep=result_dir)
		...
		for i in a[level[0]]:
			...
			# print parass['AC_dict']
			results[i], results_global[i]=loop(a, level[1:], parass, {}, {}, vers=vers, Gname=Gname)
...
def make_average_on_networks(paras, results, level_network=0):
	"""
	This is useful when simulations have been done on different networks,
	which share some characteristics. For example, some networks with increasing
	number of airports but each value of airports is represented several times.
...
if __name__=='__main__':
	...
	results, results_global = get_results(paras, vers=vers, Gname='DiskWorld')
