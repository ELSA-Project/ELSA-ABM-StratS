...
def give_airports_to_network(G, airports, distance=7, repetitions=False, force_short_computation=False):
    ...
    #G.name = 'NEWDEL1000_C_' + str(min(airports)) + '_' + str(max(airports)) + '_spatial' # TODO OOOOOOOOOOOOOOOOOOO
    G.name = name_first + str(min(airports)) + '_' + str(max(airports)) + name_last # TODO OOOOOOOOOOOOOOOOOOO
...
if fixnetwork:
    if 1:  # Take existing network
        ...
        #_name_network = 'NEWDEL1000_C'
        #_name_network = 'DEL_C_65_22'
        ...
        #G_iter=[network_whose_name_is(n) for n in ['DEL_C_4A', 'DEL_C_65_20' , 'DEL_C_4A2']]#, 'DEL_C_6A']]
            ...
            airports_iter = tuple([tuple(_pair) for _pair in airports_iter])
...
Nfp_iter = [2, 5, 10, 15, 20, 30, 50]#, 70]
...
n_iter=10                 #number of iterations in the main loop 
